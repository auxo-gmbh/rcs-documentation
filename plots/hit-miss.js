...
MATCH (:Device)-[r:SENT_TO {details: 'RESPONSE'}]->(:Device)
WITH total, count(distinct r.requestId) as hits
RETURN (hits * 1.0 / total * 1.0) * 100 as value
`

const TIME_QUERY = `
MATCH (:Device)-[r:SENT_TO {details: 'RESPONSE'}]->(:Device) 
WITH min(r.sentAt) as minTime
MATCH (:Device)-[r:SENT_TO {details: 'RESPONSE'}]->(:Device) 
WITH DISTINCT minTime, r.requestId as resId, min(r.sentAt) as timestamp
...
OPTIONAL MATCH (:Device)-[r:SENT_TO {details: 'RESPONSE'}]->(:Device)
WHERE r.sentAt<=timestamp
WITH minTime, timestamp, total, count(distinct r.requestId) as hits
...
const generateBarChartData = async () => {
    for (const alg of algorithms) {
        ...
        writeBarChartCsvFile(`data/hit_miss/${alg}.csv`, dbResult);

        const dbFailResult = await getBarChartDbResult(alg, true);
        writeBarChartCsvFile(`data/hit_miss/${alg}-fail.csv`, dbFailResult);
    }
};
...
const generateTimeChartData = async () => {
    for (const db of databases) {
        ...
        if (db.endsWith("-fail")) {
            writeTimeChartCsvFile(`data/hit_miss/${alg}-n${amountNodes}-time-fail.csv`, dbResult);
            writeTimeChartCsvFile(`data/hit_miss/${alg}-n${amountNodes}-time-fail-average.csv`, averageDbResult);
        } else {
            writeTimeChartCsvFile(`data/hit_miss/${alg}-n${amountNodes}-time.csv`, dbResult);
            writeTimeChartCsvFile(`data/hit_miss/${alg}-n${amountNodes}-time-average.csv`, averageDbResult);
        }
